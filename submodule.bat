...
@rem USE_CONFIGURATION [Debug | Release]
set USE_CONFIGURATION=Debug
...
if "%USE_PLATFORM% == "x64" (
    if not exist "third_party\openssl\win\%USE_PLATFORM%\bin\libssl-1_1-x64.dll" (
	    set "COMPILE_OPENSSL=1"
	)
) else if "%USE_PLATFORM% == "x32" (
    if not exist "third_party\openssl\win\%USE_PLATFORM%\bin\libssl-1_1.dll" (
	    set "COMPILE_OPENSSL=1"
	)
)
if %COMPILE_OPENSSL% equ 1 (
	...
	if %USE_PLATFORM% == x64 (
		perl Configure VC-WIN64A --prefix=%cd%\..\..\..\third_party\openssl\win\%USE_PLATFORM%
	) else (
		perl Configure VC-WIN32 --prefix=%cd%\..\..\..\third_party\openssl\win\%USE_PLATFORM%
	)
	if %errorlevel% neq 0 (
		goto endlocal
	)
...
set "COMPILE_CURL=0"
if "%USE_CONFIGURATION% == "Debug" (
    if not exist "third_party\curl\win\%USE_PLATFORM%\bin\libcurl-d.dll" (
	    set "COMPILE_CURL=1"
	)
) else if "%USE_CONFIGURATION% == "Release" (
    if not exist "third_party\curl\win\%USE_PLATFORM%\bin\libcurl.dll" (
	    set "COMPILE_CURL=1"
	)
)
if %COMPILE_CURL% equ 1 (
...
set "COMPILE_PROTOBUF=0"
if "%USE_CONFIGURATION% == "Debug" (
    if not exist "third_party\protobuf\win\%USE_PLATFORM%\bin\libprotobufd.dll" (
	    set "COMPILE_PROTOBUF=1"
	)
) else if "%USE_CONFIGURATION% == "Release" (
    if not exist "third_party\protobuf\win\%USE_PLATFORM%\bin\libprotobuf.dll" (
	    set "COMPILE_PROTOBUF=1"
	)
)
if %COMPILE_PROTOBUF% equ 1 (
	...
	mkdir %USE_CONFIGURATION%
	cd %USE_CONFIGURATION%
	cmake -G "Ninja" -Dprotobuf_BUILD_SHARED_LIBS=ON -DCMAKE_BUILD_TYPE=%USE_CONFIGURATION% -DCMAKE_INSTALL_PREFIX=..\..\..\..\..\..\third_party\protobuf\win\%USE_PLATFORM% ..\..
	if %errorlevel% neq 0 (
		goto endlocal
	)
	ninja -j %NUMBER_OF_PROCESSORS%
...
set "COMPILE_ZEROMQ=0"
if "%USE_CONFIGURATION% == "Debug" (
    if not exist "third_party\zeromq\win\%USE_PLATFORM%\bin\libzmq-mt-gd-4_0_10.dll" (
	    set "COMPILE_ZEROMQ=1"
	)
) else if "%USE_CONFIGURATION% == "Release" (
    if not exist "third_party\zeromq\win\%USE_PLATFORM%\bin\libzmq-mt-4_0_10.dll" (
	    set "COMPILE_ZEROMQ=1"
	)
)
if %COMPILE_ZEROMQ% equ 1 (
	...
	mkdir %USE_CONFIGURATION%
	cd %USE_CONFIGURATION%
	cmake -G "Ninja" -DCMAKE_BUILD_TYPE=%USE_CONFIGURATION% -DCMAKE_INSTALL_PREFIX=..\..\..\..\..\third_party\zeromq\win\%USE_PLATFORM% ..\..
	if %errorlevel% neq 0 (
		goto endlocal
	)
	ninja -j %NUMBER_OF_PROCESSORS%
...
if not exist "third_party\mysql\win\%USE_PLATFORM%\lib\libmysql.dll" (
	cd submodule\win
	if not exist mysql (
		mkdir mysql
	)
