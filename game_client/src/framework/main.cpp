...
int CommandHelp(const util::CWeakPointer<ArgumentBase>& arg) {
	...
	std::string strSpace;
	printf("***************************** GameClient V%u.%u *******************************\n",
		VERTION_TRUNK,VERTION_BRANCH);
	printf("Commands:\n");
...
int main(int argc, char** argv) {
	...
	if (!pConfig->LoadFile((char*)APP_CONFIG_NAME)) {
    ...
    //initialize
    CGameClient::PTR_T pInstance(CGameClient::Pointer());
    if(!pInstance->Init()){
        pInstance->Dispose();
        printf("Init() fail.\n");
        return 0;
