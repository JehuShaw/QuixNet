...
bool CGameServer::Init(int argc, char** argv) {
	...
	m_pWorkerService.SetRawPointer(new CWorkerServiceImp(strBind, strServant,
		strServerName, u16ServerId, CreatePlayer, NULL, NodeNotificationInterests));
	...
	uint16_t u16ServerRegion)
{
	...
	for(int i = 0; ; ++i) {
		...
		CAutoPointer<CallbackMFnP2<CGameServer, std::string, volatile bool> >
			...
			(s_instance, &CGameServer::KeepServersRegister, strConnect, false));
...
void CGameServer::KeepServersRegister(std::string& connect, volatile bool& bRun)
{
