	...
	if (*line_length > 0)
	{
		...
		while(char_pos < *line_length)
		{
			...
			if (curr_char == field_term_char)
			{
				...
				unsigned int final_adjustment = (line_final_char == enclosed_char) ? 1U : 0U;
				if(first_adjustment != 0U && final_adjustment == 0U) 
				{
					/* Move to the next character in the current line */
					char_pos++;
					continue;
...
void csv_parser::_read_single_line(char ** buffer, unsigned int * buffer_len)
{
    ...
    if(0 == original_pos) {
        // Ignore UTF-8 BOM
        char utf8Bom[4] = {0,0,0,0};
        fgets(utf8Bom, sizeof(utf8Bom), input_fp);
        if(utf8Bom[0] == '\xEF' 
            && utf8Bom[1] == '\xBB' 
            && utf8Bom[2] == '\xBF') 
        {
