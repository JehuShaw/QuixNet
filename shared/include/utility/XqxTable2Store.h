/*
 * File:   XqxTable2Store.h
 * Author: Jehu Shaw
 *
 * Created on 2017_7_5, 23:15
 */
#ifndef XQXTABLE2STORE_H
#define XQXTABLE2STORE_H
...
#define XQXTABLE2STORE_ALLOCATE_SIZE 8
...
private:
	...
	private:
		friend class CXqxTable2Store<T>;
...
private:
	class CIteratorBase {
	public:
		CIteratorBase(const CXqxTable2Store<T>* pXqxTable, uint32_t nIndex)
	...
	private:
		const CXqxTable2Store<T>* m_pXqxTable;
		uint32_t m_nIndex;
	};
	...
	public:
		...
		CIterator(const CXqxTable2Store<T>* pXqxTable, uint32_t nIndex)
	...
	public:
		...
		CReverseIterator(const CXqxTable2Store<T>* pXqxTable, uint32_t nIndex)
			: CIteratorBase(pXqxTable, nIndex) {
		}
...
public:
	...
	CXqxTable2Store(uint32_t nAllcSize = 0)
		: m_arrItems(NULL)
		, m_nAllcSize(0)
		, m_nCurSize(0)
	{
		if(0 == nAllcSize) {
			nAllcSize = XQXTABLE2STORE_ALLOCATE_SIZE;
		}
		AllcoTable(nAllcSize);
	}

	CXqxTable2Store(const CXqxTable2Store& orig)
	{
		if(this == &orig) {
			return;
		}
		...
		if(0 == orig.m_nAllcSize) {
			AllcoTable(XQXTABLE2STORE_ALLOCATE_SIZE);
		} else {
	...
	CXqxTable2Store& operator = (const CXqxTable2Store& right) {
	...
	void Print()
	{
		...
		//printf("/////////////////////////////////////////////////////////////////\n");
		PrintDebug("m_nCurSize = %d m_nAllcSize= %d\n", m_nCurSize, m_nAllcSize);
		for(uint32_t i = 0; i < m_nAllcSize; ++i) {
			PrintDebug("m_arrItems[%u].m_nIndex = %u", i, m_arrItems[i].m_nIndex);
			//if( i < 50000) {
