...
# USE_CONFIGURATION [Debug | Release]
export USE_CONFIGURATION=Debug
...
if { [ "$USE_CONFIGURATION" == "Debug" ] && [ ! -f "./third_party/protobuf/linux/$USE_PLATFORM/lib/libprotobufd.so" ]; } || 
{ [ "$USE_CONFIGURATION" == "Release" ] && [ ! -f "./third_party/protobuf/linux/$USE_PLATFORM/lib/libprotobuf.so" ]; }; then  
	...
	cmake -G "Unix Makefiles" -Dprotobuf_BUILD_SHARED_LIBS=ON -DCMAKE_BUILD_TYPE=$USE_CONFIGURATION -DCMAKE_INSTALL_PREFIX=../../../../../../third_party/protobuf/linux/$USE_PLATFORM ../..
	if [ $? -ne 0 ]; then
		exit 1
	fi
	make -j$(nproc)
...
if [ ! -f "./third_party/zeromq/linux/$USE_PLATFORM/lib/libzmq.so" ]; then 
...
if [ ! -f "./third_party/cppzmq/include/zmq.hpp" ]; then 
...
if [ ! -f "./third_party/mysql/linux/$USE_PLATFORM/lib/libmysqlclient.so" ]; then 
...
if { [ "$USE_CONFIGURATION" == "Debug" ] && [ ! -f "./third_party/curl/linux/$USE_PLATFORM/lib/libcurl-d.so" ]; } || 
{ [ "$USE_CONFIGURATION" == "Release" ] && [ ! -f "./third_party/curl/linux/$USE_PLATFORM/lib/libcurl.so" ]; }; then  
	...
	mkdir $USE_CONFIGURATION
	cd $USE_CONFIGURATION
	cmake -G "Unix Makefiles" -DCMAKE_BUILD_TYPE=$USE_CONFIGURATION -DCMAKE_INSTALL_PREFIX=../../../../../third_party/curl/linux/$USE_PLATFORM ../..
	if [ $? -ne 0 ]; then
		exit 1
	fi
	make -j$(nproc)
